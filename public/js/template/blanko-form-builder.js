...
function addEvents(elem, id = null, signature = null){
  ...
  $(elem).find('.drag-options .help-text').keyup(function(e){
    ...
    if(e.keyCode == 8){
      ...
      drange = d.getRangeAt(0);
      if(drange.startContainer.tagName == "LI" || drange.startContainer.toString() == ""){
        drange.startContainer.remove();
        textnode = document.createElement('br');
        drange.insertNode(textNode);
      }
    }
...
function formatText(element, type){
  ...
  //       range.insertNode(document.createTextNode(c.textContent));
  //       //range.innerHTML = t.replace(/<\/?[^>]+(>|$)/g, "");
  //     }
  //   }
  // }
  ...
  if (window.getSelection) {
    ...
    if (sel.rangeCount) {
      if(type != "clear"){
        if(sel.anchorNode.tagName != type || range.startContainer.tagName != undefined){
          range.surroundContents(tag);
        }
      }else {
        r = [];
