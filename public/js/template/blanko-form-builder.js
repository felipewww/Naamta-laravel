...
function addEvents(elem, id = null){
  ...
  $(elem).find('.drag-options .add-options').click(function(e){
    ...
    if(label.val() != "" && value.val() != ""){
      addOption( type , field, label.val(), value.val());
      label.val('');
      value.val('');
    }
  });
  ...
  if(type == "signature"){
    ...
    canvas.className = "update-required";
    canvas.height = 120;
    dragInput.append(canvas);
