...
function formatText(element, type){
  ...
  var positionStart = sel.getRangeAt(0).startOffset;
  var positionEnd = sel.getRangeAt(0).endtOffset;

  value = element.innerHTML;
  if(positionStart != positionEnd){
    var output;

    a = value.slice(0, positionEnd);
    b = a.lastIndexOf(">");
    a = value.slice(b, positionEnd);

    output = value.slice(a , tagEnd, value.slice(positionEnd));
    console.log(output);


    // output = [output.slice(0, positionStart), tagStart , output.slice(positionStart)].join('');
    // console.log(output);
